	...
	(do ((k 0 (incf k)))
	  ...
	  (do ((l 0 (incf l)))
	    ...
	    (let ((unfiltered (copy-list '(0 0 0 0 0))))
	      ...
	      (push unfiltered acc)))))))
    (let ((actions (mapcar #'(lambda (action)
			       (if (listp (first action))
				   (remove-if #'(lambda (a)
						  (if (eq (butlast a)
							  (butlast action))
						      t))
...
(defun make-transitions (mdpr)
  (let ((l (make-list 100 :initial-element 'x)))
    ...
    (mapcar #'(lambda (action)
		(let ((res (make-action-list (mdpr-actions mdpr) 
					     l
					     (- (length (mdpr-actions mdpr)) 
						(position action (mdpr-actions mdpr))))))
		  (setq action (cons action (second res)))))
	    (mdpr-actions mdpr))))
; mdpr-actions = mdpr actions
; ll = list used to figure probabilities
; numactions = used to assign probability to last remaining action
(defun make-action-list (mdpr-actions ll numactions)
  (let ((l (make-list 100 :initial-element 'x))
	(res-base (percentage-piece ll numactions)))
    (list (first res-base) 
	  (mapcar #'(lambda (action)
		      (let ((res (percentage-piece l (- (length mdpr-actions) 
							(position action mdpr-actions)))))
			(setq l (first res))
